...
namespace SI_Projekt{
    class NodeV1 {
        public NodeV1(char letter) {
            this.counter = 0;
            this.total = 0;
            this.letter = letter;
            this.children = new NodeV1[27];
        }
        ...
        private void teachNewWord(string word, int letterPointer) {
            foreach (NodeV1 child in children)
                child.total++;
        ...
        private void createNextLevels(int depth) {
            if (depth == 0) return;
            NodeV1[] children = createNewLevel();
            for (int i = 0; i < 27; i++) 
                this.children[i].children = children;
            this.children[0].createNextLevels(depth - 1);
        }
        
        private NodeV1[] createNewLevel() {
            NodeV1[] children = new NodeV1[27];
            for (int i = 0; i < 27; i++) {
                if (i == 26)
                    children[i] = new NodeV1('\0');
                else
                    children[i] = new NodeV1((char)(0x61 + i));
                if (i != 0)
        ...
        private NodeV1[] children { get; set; }
        private NodeV1 brother { get; set; }
    }
}
